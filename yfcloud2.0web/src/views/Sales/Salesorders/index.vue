    ...
    <el-main id="elmain">
      ...
      <div class="findBox" :style="popoverStyle" v-show="popoverState" v-on:click.stop="1==1">
        <el-table :data="materielData" :height="250" @row-click="rowClick" border>
          <el-table-column prop="dicCode" label="包型代码" />
          <el-table-column prop="dicValue" label="包型名称" />
          <!-- <el-table-column prop="mMaterialFile.spec" label="规格型号" /> -->
          <!-- <el-table-column prop="mMaterialFile.colorName" label="颜色" /> -->
          <!-- <el-table-column prop="mMaterialFile.baseUnitName" label="基本计量单位" /> -->
      ...
      <div>
        ...
        >
          ...
          <el-table-column prop="materialCode" label="包型代码" width="140">
            <template slot="header">
              <span class="tableHeader">
          ...
          <el-table-column prop="materialName" label="包型名称" width="140">
            <template slot="header">
              <span class="tableHeader">
                <span class="start">*</span>
                <span>包型名称</span>
          ...
          <el-table-column prop="defaultWarehouseName" label="配色方案" width="140">
            <template slot="header">
              <span class="tableHeader">
                <span class="start"></span>
                <span>配色方案</span>
...
export default {
  name: "viewsSalesSalesordersindexvue",
  mixins: [Receipt],
  data() {
    return {
      warehouseData: [],
  ...
  async mounted() {
    ...
    if (this.$route.params.type == 1 || this.$route.params.type == 2) {
      ...
      var code = Storage.LStorage.get("Salescode");
      if (!code) {
        this.getCode();
      } else {
  ...
  methods: {
    ...
    async WarehouseChange(item) {
      ...
      this.doItem.salesRate.value =
        MaterialData.salesRate == null || MaterialData.salesRate == ""
          ? 1
          : MaterialData.salesRate;
    ...
    getWarehouseData(packageCode, packageName) {
      ...
      return new Promise(function(reslove, reject) {
        ...
        request({
          url: "/manufacturing/api/TMMColorSolutionMain/GetColorSolution",
          method: "get",
    ...
    async rowClick(row) {
      ...
      this.doItem.defaultWarehouseName.valid = false;
      ...
      // this.doItem.goodsCode.value = row.dicCode;
    ...
    OnBtnSaveSubmit(row) {
      ...
      request({
        ...
        if (res.code === 0) {
          ...
          request({
            url: "sales/api/TSSMSalesOrderMain/GetDetailList",
    ...
    setTableData(dt) {
      ...
      setTimeout(() => {
        // this.tableData = [...listArr, ...this.setTable(this.tableLen)];
    ...
    doOtherWhAudit(state) {
      ...
      this.$confirm(tip, "确认信息", {
        ...
        .then(() => {
          this.fullscreenLoading = true;
          request({
            url: "/sales/api/TSSMSalesOrderMain/SalesOrderAudit",
            method: "put",
            data: data
          }).then(res => {
            if (res.code === 0) {
