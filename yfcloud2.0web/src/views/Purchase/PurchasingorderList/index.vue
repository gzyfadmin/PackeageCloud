  ...
  >
    <el-header id="elheader" class="header headerBd" height="auto">
      <el-form :label-position="labelPosition" label-width="76px" inline class="FormInput">
        ...
        <el-form-item label="采购订单号：" label-width="90px">
          <el-input
    ...
    <el-main v-if="showtab"  class="wmltable" id="elmain">
      <el-table
        ref="table"
        :data="tableData"
        style="width: 99.9%"
      ...
      >
        ...
        <el-table-column prop="orderNo" label="采购订单号" width="150px" sortable="custom">
          <template slot-scope="scope">
        ...
        <el-table-column label="采购员" width="150px">
          <template slot-scope="scope">
        ...
        <el-table-column label="采购方式" width="150px">
          <template slot-scope="scope">
...
export default {
  name: "viewsPurchasePurchasingorderListindexvue",
  ...
  async activated() {
    ...
    setTimeout(() => {
      this.showtab = true;
      this.setStyle();
      this.pathData ="viewsPurchasePurchasingorderindexvue";
  ...
  methods: {
    getSummaries(param) {
      ...
      const values0 = data.map(item => {
        var accountNum = 0;
        accountNum = item.purchaseAmount + accountNum;
    ...
    addPutInStorage(item) {
      ...
      var currData = {
        id: item.id,
        _LogName: `删除ID：${item.id} 的采购单`
      };
      var data = RequestObject.CreatePostObject(currData);

      this.$confirm("是否删除采购单？", "确认信息", {
        type: "warning"
      })
        .then(() => {
          this.postData(data);
    ...
    getMainList() {
      ...
      if (this.dtData.supplierName != "") {
        list.push({
          column: "orderNo",
          content: trim(this.dtData.supplierName),
          condition: 6
        });
      }
