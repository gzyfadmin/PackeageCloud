    ...
    <el-main>
      ...
      >
        ...
        <el-table-column label="账存数量" width="150">
          <template slot-scope="scope">
            <div>{{Math.floor(scope.row.accountNum)}}</div>
          </template>
        </el-table-column>

        <el-table-column label="实存数量" width="150">
          <template slot-scope="scope">
            <div>{{Math.floor(scope.row.actualNum)}}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="盘盈数量" width="150">
          <template slot-scope="scope">
            <div>{{Math.floor(scope.row.profitNum)}}</div>
          </template>
        </el-table-column>
        <el-table-column label="盘亏数量" width="150">
          <template slot-scope="scope">
            <div>{{Math.floor(scope.row.deficitNum)}}</div>
          </template>
        </el-table-column>

        <el-table-column label="基本单位账存数量" width="150">
          <template slot-scope="scope">
            <div>{{Math.floor(scope.row.accountUnitNum)}}</div>
          </template>
        </el-table-column>
        <el-table-column label="基本单位实存数量" width="150">
          <template slot-scope="scope">
            <div>{{Math.floor(scope.row.actualUnitNum)}}</div>
          </template>
        </el-table-column>
        <el-table-column label="基本单位盘盈数量" width="150">
          <template slot-scope="scope">
            <div>{{Math.floor(scope.row.profitUnitNum)}}</div>
          </template>
        </el-table-column>
        <el-table-column label="基本单位盘亏数量" width="150">
          <template slot-scope="scope">
            <div>{{Math.floor(scope.row.deficitUnitNum)}}</div>
          </template>
        </el-table-column>
      ...
      request({
        ...
        if (res.code === 0) {
          ...
          for (var i = 0; i < res.data.length; i++) {
            if (
              res.data[i].materialName == trim(this.dtData.materialname) ||
