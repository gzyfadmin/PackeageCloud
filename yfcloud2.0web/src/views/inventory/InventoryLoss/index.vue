  ...
  >
    <el-header id="elheader" class="header headerBd" height="auto">
      ...
      <el-form :label-position="labelPosition" inline class="FormInput">
        <el-form-item label="盘亏出库单号：" label-width="105px" prop="warehousingType">
          <el-input
    ...
    <el-main class="wmltable" id="elmain">
      ...
      >
        ...
        <el-table-column prop="whSendDate" label="盘亏出库日期" width="200px" sortable="custom">
          <template slot-scope="scope">{{ scope.row.whSendDate|formatTDate }}</template>
        </el-table-column>
        <el-table-column prop="whSendOrder" label="盘亏出库单号" width="200px" sortable="custom">
          <template slot-scope="scope">
            <div class="link" @click="goPage(scope.row,1)">{{ scope.row.whSendOrder }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="100">
          <template slot-scope="scope">
            ...
            >
              <!-- <el-button
                v-if="btnAip.edit.buttonCaption&&scope.row.isShowEdit"
                  type="primary"
                  icon="el-icon-edit"
                  circle
...
export default {
  name: "viewsinventoryInventoryLossindexvue",
  components: {
    Pagination
  },
  filters: {
  ...
  async activated() {
    ...
    this.pathData = "viewsinventoryInventoryLossSlipindexvue";
  },
  mounted() {
    this.setStyle();
    this.showtab = false;
    ...
    // this.pathDataView = getName("/HHHPyK", this.$store.getters.userPermission);
    this.pathData = "viewsinventoryInventoryLossSlipindexvue";
  },
  methods: {
    goPage(item, type) {
