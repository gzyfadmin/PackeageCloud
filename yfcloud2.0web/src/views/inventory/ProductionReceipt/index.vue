      ...
      <div>
        ...
        >
          ...
          <el-table-column prop="workshopName" label="生产车间" width="120">
           ...
           <el-table-column prop="siteName" label="生产场地" width="120">
            <template slot="header">
              <span class="tableHeader">
                <span>生产场地</span>
              </span>
...
export default {
  name: "viewsinventoryProductionReceiptindexvue",
  mixins: [Receipt],
  filters: {
    formatTDate: value => {
      if (value == "") {
  ...
  async mounted() {
    ...
    this.workshopOption = await this.getWorkshopOption();//生产场地
    this.siteOption = await this.getSiteOption();//生产场地
    ...
    if (this.$route.params.type) {
      ...
      var code = Storage.LStorage.get("ProductionReceiptCode");
      if (!code) {
        this.getCode();
      } else {
  ...
  methods: {
    ...
    getSiteOption() {
      return new Promise(function(reslove, reject) {
        var para = {
          ...
          pageIndex: 0,
          queryConditions: [
            { column: "TypeName", content: "生产场地", condition: 0 }
          ],
          orderByConditions: null
        };
        request({
    ...
    blurCheck(itemList, item, type, state) {
      ...
      switch (type) {
        case 1: //物料代码
          ...
          break;
        case 2: //实收数量
          this.PaidInBlur(itemList); //验证实收数量
          break;
        case 3: //单价
    ...
    getCode() {
      request({
        url: "warehouse/api/TWMProductionWhMain/GetOrderNo",
        method: "GET"
      }).then(res => {
        if (res.code == -1) {
          this.dtData.warehousingOrder = "";
          ...
          Storage.LStorage.set("ProductionReceiptCode", res);
        }
        // this.dtData.warehousingOrder = res;
    ...
    async addPutInStorage(state) {
        ...
        checkSelect.map(item => {
          ...
          if (item.editState) {
            ...
            if(item.workshopName.key!="") {
              param.workshopId = item.workshopName.key;
            }
            if(item.siteName.key!="") {
              param.siteId = item.siteName.key;
            }
            if (item.proOrderDetailId != "") {
              param.proOrderDetailId = item.proOrderDetailId;
            }
      ...
      if (state == 3) {
        ...
        var currData = {
          ...
          _LogName: `删除ID：${this.selectRow.id} 的生产入库单`
        };
      ...
      request({
        ...
        if (res.code === 0) {
          ...
          if (state == 1) {
            ...
            Storage.LStorage.remove("ProductionReceiptCode");
          }
          if (state == 3) {
            this.tableData = [];
            this.resetDeafultData();
    ...
    chooseNumber() {
      ...
      this.$refs.DocumentNo.dtData.tranSourceCode = "";
    ...
    OnBtnSaveDocument(row, state) {
      this.fullscreenLoading = true;
      this.dtData.orderNo = row.tranSourceCode;
      this.dtData.orderId = row.id;
      this.dtData.tranSourceId = row.tranSourceId;
      ...
      if (!state) {
        var list = [
          {
            column: "tranSourceId",
            content: row.tranSourceId,
            condition: 0
          }
        ];
        var rqs = RequestObject.CreateGetObject(false, 0, 0, list);
        request({
          url: "manufacturing/api/TMMProductionOrderMain/GetOrderResult",
          method: "GET",
          params: {
            requestObject: JSON.stringify(rqs)
          }
          ...
          if (res.code === 0) {
            if (res.data.length > 0) {
              this.setTableData2(res.data[0].childList, res.data[0]);
            }
          } else {
            this.$confirm(res.info, "错误信息", {
              confirmButtonText: "确定",
    ...
    setTableData(dt, row) {
      ...
      dt.map(item => {
        var list = {
          ...
          editState: false,
          rowIndex: 0,
          proOrderDetailId: item.proOrderDetailId,
          materialCode: {
            // 物料代码
            id: newGuid(),
    ...
    doOtherWhAudit(state) {
      ...
      this.$confirm(text, {
        ...
        .then(() => {
          ...
          this.fullscreenLoading = true;
          request({
            url: "/warehouse/api/TWMProductionWhMain/Audit",
            method: "put",
            data: data
          }).then(res => {
            if (res.code === 0) {
