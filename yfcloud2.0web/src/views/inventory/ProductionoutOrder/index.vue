    ...
    <el-header id="elheader" class="header headerBd" height="auto">
      <div class="middleWidth">
        ...
        >
          ...
          <el-form-item class="formItem cursor" label="源单号：" prop="orderNo">
            <el-input
          ...
          <el-form-item class="formItem" label="收货地址：" prop="receiptAddress">
            <el-input
              class="headerIp"
    ...
    <el-main id="elmain">
      ...
      <div>
        ...
        >
          ...
          <el-table-column prop="PaidIn" label="实发数量" width="120">
            <template slot="header">
              <span class="tableHeader">
                <span class="start">*</span>
                <span>实发数量</span>
          ...
          <el-table-column prop="account" label="可用数量" width="120">
            <template slot="header">
              <span class="tableHeader">
                <span>可用数量</span>
              </span>
          ...
          <el-table-column prop="ShouldbeSReceiveNum" label="可发数量" width="120">
            <template slot="header">
              <span class="tableHeader">
                <span>可发数量</span>
              </span>
...
export default {
  name: "viewsinventoryProductionoutOrderindexvue",
  filters: {
    formatTDate: value => {
      if (value == "") {
        return "";
  ...
  methods: {
    ...
    clickEvent() {
      ...
      //hihi
      // console.log(this.dtData.orderNo);
      if (!this.dtData.orderNo) {
        // this.$message.error("请先选择源单号")
        this.$message({
          showClose: true,
          message: "请先选择源单号",
          type: "warning"
    ...
    async computeAccount(item) {
      // 获取账存数量
      if (item.materialCode.key != "" && item.defaultWarehouseName.key != "") {
        this.fullscreenLoading = true;
        ...
        var warehouseRate = isEmpty(item.warehouseRate) ? 1 : item.warehouseRate;
        if (item.ShouldbeSReceiveNum.value > item.account.value) {
          item.PaidIn.value = item.account.value;
          item.baseUnitNumActualReceived.value = BigNumber(item.account.value).multipliedBy(warehouseRate).toNumber();
        } else if (item.ShouldbeSReceiveNum.value < item.account.value) {
          item.PaidIn.value = item.ShouldbeSReceiveNum.value;
          item.baseUnitNumActualReceived.value = BigNumber(item.ShouldbeSReceiveNum.value).multipliedBy(warehouseRate).toNumber();
        } else if (item.ShouldbeSReceiveNum.value == item.account.value) {
          item.PaidIn.value = item.ShouldbeSReceiveNum.value;
          item.baseUnitNumActualReceived.value = BigNumber(item.ShouldbeSReceiveNum.value).multipliedBy(warehouseRate).toNumber();
        }
      ...
      if (state == 1 || state == 2) {
        ...
        checkSelect.map(item => {
          ...
          if (item.editState) {
            ...
            var param = {
              id: id_,
              pickApplyDetailId:item.pickApplyDetailId,
        ...
        // }
        if (!accountFlag) {
          this.$message.error("实发数量不能大于可发数量或可用数量");
          return;
        }
    ...
    chooseNumber() {
      ...
      this.$refs.DocumentNo.tableFlag = false;
      setTimeout(() => {
        this.$refs.DocumentNo.tableFlag = true;
      }, 0);
    ...
    OnBtnSaveDocument(row, state) {
      ...
      if (!state) {
        ...
        request({
          ...
          url: "manufacturing/api/TMMPickApplyMain/GetTransWareDate",
      ...
      dt.map(item => {
        var list = {
          ...
          ShouldbeSReceiveNum: {
            // 应发数量
            id: newGuid(),
            value: item.warehouseUnitTransNum,
            isShow: false,
            valid: false
          },
          // batchNumber: {
    ...
    setTableData2(dt, row) {
      ...
      dt.map(item => {
        var list = {
          ...
          editState: false,
          rowIndex: 0,
          warehouseRate: isEmpty(item.warehouseRate) ? 1 : item.warehouseRate,
          pickApplyDetailId: item.id,
          materialCode: {
            // 物料代码
            id: newGuid(),
            value: item.materialCode,
