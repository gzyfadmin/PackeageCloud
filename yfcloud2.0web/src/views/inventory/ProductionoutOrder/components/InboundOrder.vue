          ...
          <el-form ref="dtData" label-width="76px" class="FormInput" inline label-position="left">
            <el-form-item label="出库单号：">
              <el-input
                ...
                clearable
                style="width:320px"
                @keyup.enter.native="getMainListIndex(pageIndex=1)"
                @clear="getMainListIndex(pageIndex=1)"
              >
                <i slot="suffix" class="el-input__icon el-icon-search" @click="getMainListIndex(pageIndex=1)"></i>
              </el-input>
            </el-form-item>
            <el-form-item label="出库时间：">
              <el-date-picker
                ...
                @change="getMainListIndex(pageIndex=1)"
              />
            </el-form-item>
            <el-form-item label="审核状态：">
              <el-radio-group v-model="dtData.auditStatus" @change="getMainListIndex(pageIndex=1)">
...
export default {
  ...
  methods: {
    ...
    getMainList() {
      ...
      var orderConditions = [];
      var list = [];
      if(this.dtData.warehousingOrder!=null) {
        list.push({
          column: "whSendOrder",
       ...
       if (this.dtData.whSendDate != null) {
        if (this.dtData.whSendDate[0]) {
          ...
          list.push({
            column: "whSendDate",
            content: this.dtData.whSendDate[0]
              ? startDate + "," + endDate
              : "",
            condition: 10
          });
      ...
      this.tableData = [];
      request({
        // /api/TWMProductionMain/GetMainList
