...
export default {
  ...
  async mounted() {
    ...
    if (this.$route.params.type) {
      ...
      var code = Storage.LStorage.get("SalesReceiptCode1");
      if (!code) {
        this.getCode();
      } else {
...
      }
    ...
    getCode() {
      request({
        ...
        if (res.code == -1) {
          ...
          Storage.LStorage.set("SalesReceiptCode1", res);
        }
        // this.dtData.warehousingOrder = res;
      ...
      request({
        ...
        if (res.code === 0) {
          ...
          if (state == 1) {
            ...
            Storage.LStorage.remove("SalesReceiptCode1");
          }
          if (state == 3) {
            this.tableData = [];
            this.resetDeafultData();
    ...
    OnBtnSaveDocument(row, state) {
      ...
      if (!state) {
        request({
          url: "sales/api/TSSMSalesOrderMainNoPackage/GetTransferList",
          method: "GET",
