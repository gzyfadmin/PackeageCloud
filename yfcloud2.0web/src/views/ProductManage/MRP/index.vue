<template>
  ...
  >
    ...
    <el-main>
      ...
      <div class="rightBox" :style="'height:'+tableHeight+'px'" ref="print">
        <!-- 头部按钮区开始 -->
        ...
        <div class="btnHeader" id="btnHeader">
          <div v-if="btnAip.add" class="btnHeaderBox">
            <el-button
              v-if="btnAip.calmaterial&&btnAip.calmaterial.buttonCaption"
              :disabled="!mrpStatus"
              type="default"
              @click="doTrans(1)"
            >{{ btnAip.calmaterial.buttonCaption }}</el-button>
            <el-button
              v-if="btnAip.createpick&&btnAip.createpick.buttonCaption"
              :disabled="!isPick"
              type="default"
              @click="doTrans(2)"
            >{{ btnAip.createpick.buttonCaption }}</el-button>
            <el-button
              v-if="btnAip.createorder&&btnAip.createorder.buttonCaption"
              type="default"
              :disabled="!isPurchase"
              @click="doTrans(3)"
            >{{ btnAip.createorder.buttonCaption }}</el-button>
            <el-button
              v-if="btnAip.againpicking&&btnAip.againpicking.buttonCaption"
              type="default"
              @click="doTrans(4)"
            >{{ btnAip.againpicking.buttonCaption }}</el-button>
            <el-button
              type="default"
        ...
        <el-header id="elheader" class="header" height="auto">
          <div>
            ...
            >
              ...
              <el-form-item class="formItem" label="订单日期：">
                <div class="headerIp bbfe6">{{headerDataObj.orderDate|formatTDate}}</div>
              </el-form-item>
              <el-form-item class="formItem" label="订单单号：">
                <div class="headerIp bbfe6">{{headerDataObj.productionNo}}</div>
              </el-form-item>
              <el-form-item class="formItem" label="审核状态：">
                <div class="headerIp bbfe6">
                  <div v-if="headerDataObj.auditStatus==0">待审核</div>
                  <div v-if="headerDataObj.auditStatus==1">未通过</div>
                  <div v-if="headerDataObj.auditStatus==2">通过</div>
                  <div v-if="headerDataObj.auditStatus==3">撤销审核</div>
                </div>
              </el-form-item>
              <el-form-item class="formItem" label="操作员：">
                <div class="headerIp bbfe6">{{headerDataObj.operatorName}}</div>
              </el-form-item>
              <el-form-item class="formItem" label="审核员：">
                <div class="headerIp bbfe6">{{headerDataObj.auditName}}</div>
              </el-form-item>
              <el-form-item class="formItem" label="审核时间：">
                <div class="headerIp bbfe6">{{headerDataObj.auditTime|formatTDate}}</div>
              </el-form-item>
              <el-form-item class="formItem" label="入库申请单号：">
                <div class="headerIp bbfe6">{{dtData.whApplyMain | dataFormat}}</div>
              </el-form-item>
              <el-form-item class="formItem" label="采购申请单号：">
                <div class="headerIp bbfe6">{{dtData.purchase | dataFormat}}</div>
              </el-form-item>
              <el-form-item class="formItem" label="领料单号：">
                <div class="headerIp bbfe6" style="width:540px;">{{dtData.pickModel | dataFormat}}</div>
              </el-form-item>
        ...
        <!-- table区域开始 -->
        <!-- <div id="headerTable">
          <el-table :data="headerData" :height="75" :highlight-current-row="false">
        ...
        <div style="height:41px;">
          <el-tabs type="card" :closable="false" @tab-click="tabClick" v-model="tabPage">
            <el-tab-pane
              :key="item.id"
              ...
              :name="item.id"
            >
              <span slot="label" class="tabLabel">{{item.title}}</span>
            </el-tab-pane>
          </el-tabs>
        </div>
        <transition name="el-fade-in-linear">
          <div v-show="transitionFlag">
            <div @click.stop="1==1" v-show="transitionFlag&&tabPage==1">
              <el-table
                id="tableData"
                ref="table"
                :height="tableHeight3"
              ...
              >
                ...
                <el-table-column prop="colorSolutionCode" label="配色方案名称" width="200">
                  <template slot="header">
                    <span class="tableHeader">
                      <span>配色方案名称</span>
                    </span>
                ...
                <el-table-column prop="singleValue" label="单个用量" width="100">
                  <template slot="header">
                    <span class="tableHeader">
                      <span>单个用量</span>
                    </span>
            ...
            <div v-show="transitionFlag&&tabPage==2">
              <Details ref="Details" :tableHeight3.sync="tableHeight3" />
            </div>
          </div>
        </transition>
        <!-- table区域结束 -->
    ...
    <Preview
      ...
      title="导出"
      :dtData.sync="dtData"
      :headerDataObj.sync="headerDataObj"
...
export default {
  name: "viewsProductManageMRPindexvue",
  filters: {
    formatTDate: value => {
      if (value == "") {
        return "";
  ...
  methods: {
    doPreview() {
      this.$refs["Preview"].open(2);
      this.$refs["Preview"].setData(this.DetailData);
    },
    doPrint() {
      this.$refs["Print"].open(2);
      this.$refs["Print"].setData(this.DetailData);
    },
    tabClick(data) {
      if (this.selectState == data.$attrs.id) return;
      this.selectState = data.$attrs.id;
    ...
    setStyle() {
      ...
      this.$nextTick(() => {
        ...
        this.tableHeight2 = b - h - 40 - 40 - btn;
      });
    },
    // 设置Tab切换开始
    onKeydown(event) {
    ...
    chooseNumber() {
      ...
      this.$refs.DocumentNo.dtData.auditStatus = 0;
    ...
    OnBtnSaveDocument(row, state) {
      ...
      this.tableData = [];
      this.$refs["Details"].setDetailData([]);
      // this.$refs["Details"].setDetailData([]); //设置详情
      this.dtData.productionNo_SC = row.productionNo;
      this.getProductionList();
      this.headerDataObj = {};
      this.headerDataObj = row;
      ...
      // if (!state) {
        request({
          ...
          if (res.code === 0) {
            this.setTableData2(res.data.mainList.summary, row);
            this.DetailData = await this.$refs["Details"].setDetailData(
              res.data.mainList.deatails
            ); //设置详情
    ...
    doTrans(state) {
      ...
      var URL = "";
      if (state == 1) {
        URL = "manufacturing/api/MRP/AutoComputeMRP";
      }
      if (state == 2) {
      ...
      if (state == 4) {
        URL = "manufacturing/api/MRP/RePick";
      }
      this.fullscreenLoading = true;
      request({
        url: URL,
        ...
        if (res.code === 0) {
          if (state == 1) {
            ...
            this.setTableData2(res.data.summary);
            this.DetailData = await this.$refs["Details"].setDetailData(
              res.data.deatails
            ); //设置详情
            this.fullscreenLoading = false;
    ...
    getProductionList() {
      ...
      if (this.dtData.productionNo_SC != "") {
        list.push({
          column: "productionNo",
          content: trim(this.dtData.productionNo_SC),
          condition: 0
        });
      }
      ...
      request({
        ...
        if (res.code === 0) {
          ...
          var data = this.productionData[0];
          if (data) {
            this.dtData.whApplyMain = data["whApplyMain"]
              ? data["whApplyMain"]
              : "";
            this.dtData.pickModel = data["pickModel"] ? data["pickModel"] : "";
            this.dtData.purchase = data["purchase"] ? data["purchase"] : "";
          }
