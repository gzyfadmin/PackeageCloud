        ...
        <el-col :span="20">
          <el-header id="elheader" class="header headerBd" height="auto">
            ...
            <div v-if="btnAip">
              <el-button
                ...
                type="primary"
                :disabled="!nowclick.actualId||!colorAddFlag"
                @click="right_list(pageIndex=1)"
          ...
          <div id="e" style="overflow-y:auto;width: 100%;" :style="{'height':mainHeight+'px'}">
            ...
            <el-table
              ...
              border
              style="width: 100%;"
              :max-height="mainHeight-3"
            >
              <el-table-column prop="solutionCode" label="方案编号" width="120" fixed="left" />
              <!-- min-width="120" -->

              <div v-for="(item,index) in newitem.id" :key="index">
                <el-table-column
                  filterable
                  v-if="index!=newitem.id.length-1"
                  width="120"
                  :label="newitem.label[index]"
                  :prop="'itemcaption'+newitem.id[index]"
                />
                <el-table-column
                  v-if="index==newitem.id.length-1"
                  min-width="120"
              ...
              <el-table-column v-if="!newitem.id" fixed="left" label />
...
export default {
  // name: "DataDictionary",
  name: "ViewsProductColorscheme_bakindexvue",
  components: {
    UploadPic,
    Pagination
  },
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      // hihi
      nowchildren.map(item => {
        var str = "itemid" + item.chooseid;
        var obj = new Object({
    ...
    async right_list() {
      ...
      // this.tableData = [];
      var nowarr = await this.asyncRandom(
        this.clickNodes,
        this.form0.ParentName
      );
      if (nowarr) {
        var childrenarr = [];
        var newobj = [];
        nowarr.map(item => {
          childrenarr.push(item.chooseid);
          var obj = new Object({
            ParentId: item.ParentId,
            ParentName: item.ParentName,
            actualId: item.actualId,
        ...
        this.form0.oldChildren = [];
        this.form0.oldChildren = childrenarr; // 配色项目ID
        this.form0.children = childrenarr; // 配色项目ID
        this.form0.children2 = [];
        this.form0.children2 = newobj;
      }

      await this.getTMMColorItem(list);
      this.tableData = [];
      var list = [
        { column: "packageid", content: this.clickNodes, condition: 0 }
      ];
    ...
    async getorderTypeoption(e) {
      ...
      var para = {
        ...
        pageIndex: 0,
        queryConditions: [
          { column: "TypeName", content: "配色项目", condition: 0 }
        ],
        orderByConditions: null
      };
      request({
      ...
      var list = [];
      // list.push({ column: "typename", content: "品牌设置", condition: 0 });

      // if (this.dicValue0) {
      //   list.push({
      //     column: "dicValue",
      //     content: this.dicValue0,
      //     condition: 6
      //   });
      // }
          ...
          async function getlist() {
            for (let i = 0; i < that.orderTypeoption.length; i++) {
              ...
              if (item.id) {
                var obj = new Object({
                  actualId: item.id,
                  deptName: item.dicCode + item.dicValue,
                  children: newarr2,
                  ParentId: item.id,
                  ParentName: item.dicValue,
                  state: 1
                });
    ...
    setStyle() {
      ...
      this.treeDivStyle = `height:${this.mainHeight}px`;
      // this.treeStyle =
      //   "width:256px;height:" +
