<template>
  <el-container id="MainBill" class="Wsty">
    <!-- 右边列表框开始 -->
    <!-- {{packageId}} -->
    <div style="width:100%;">
      <!-- table开始 -->
      ...
      >
        ...
        <el-table-column prop="materialName_" label="物料名称" width="150">
          <template slot="header">
            <span class="tableHeader">
              <span class="start">*</span>
              <span>物料名称</span>
          ...
          <template slot-scope="scope">
            ...
            <EditInput
              v-model="scope.row.materialName_"
              @clickEvent="clickEvent"
              @clickEventAfter="clickEventAfter"
              @blurCheck="blurCheck"
              type="materialName_"
        ...
        <el-table-column prop="materialName" label="部位名称" width="150">
          ...
          <template slot-scope="scope">
            ...
            >
              ...
              <el-select
                v-show="scope.row.materialName.isShow"
                v-model="scope.row.materialName.key"
                :id="scope.row.id+'___materialName'"
                placeholder="请选择"
                @change="WarehouseChange(scope.row.materialName)"
              >
                <el-option
        ...
        <el-table-column prop="lengthValue" label="长" width="90">
          ...
          <template slot-scope="scope">
            <!-- <div
              @click="chengenum($event,scope.row,'lengthValue')"
              :class="{validStyle:scope.row.lengthValue.valid}"
            >
              <div
                class="tableTd"
                v-show="!scope.row.lengthValue.isShow"
              >{{scope.row.lengthValue.value}}</div>
              <el-input
                v-show="scope.row.lengthValue.isShow"
                v-model="scope.row.lengthValue.value"
                :id="scope.row.id+'___lengthValue'"
                size="mini"
                @blur="NumBlur(scope.row.lengthValue,scope.row)"
                @keyup.enter.native="NumBlur(scope.row.lengthValue,scope.row)"
              ></el-input>
            </div>-->
            <EditInput
        ...
        <el-table-column prop="widthValue" label="宽" width="90">
          ...
          <template slot-scope="scope">
            <!-- <div
              @click="chengenum($event,scope.row,'widthValue')"
              :class="{validStyle:scope.row.widthValue.valid}"
            >
              <div
                class="tableTd"
                v-show="!scope.row.widthValue.isShow"
              >{{scope.row.widthValue.value}}</div>
              <el-input
                v-show="scope.row.widthValue.isShow"
                v-model="scope.row.widthValue.value"
                :id="scope.row.id+'___widthValue'"
                size="mini"
                @blur="NumBlur(scope.row.widthValue,scope.row)"
                @keyup.enter.native="NumBlur(scope.row.widthValue,scope.row)"
              ></el-input>
            </div>-->
            <EditInput
        ...
        <el-table-column prop="numValue" label="件数" width="90">
          ...
          <template slot-scope="scope">
            <!-- <div
              @click="chengenum($event,scope.row,'numValue')"
              :class="{validStyle:scope.row.numValue.valid}"
            >
              <div class="tableTd" v-show="!scope.row.numValue.isShow">{{scope.row.numValue.value}}</div>
              <el-input
                v-show="scope.row.numValue.isShow"
                v-model="scope.row.numValue.value"
                :id="scope.row.id+'___numValue'"
                size="mini"
                @blur="NumBlur(scope.row.numValue,scope.row)"
                @keyup.enter.native="NumBlur(scope.row.numValue,scope.row)"
              ></el-input>
            </div>-->
            <EditInput
        ...
        <el-table-column prop="wideValue" label="封度（宽幅）" width="100">
          ...
          <template slot-scope="scope">
            <!-- <div
              @click="chengenum($event,scope.row,'wideValue')"
              :class="{validStyle:scope.row.wideValue.valid}"
            >
              <div
                class="tableTd"
                v-show="!scope.row.wideValue.isShow"
              >{{scope.row.wideValue.value}}</div>
              <el-input
                v-show="scope.row.wideValue.isShow"
                v-model="scope.row.wideValue.value"
                :id="scope.row.id+'___wideValue'"
                size="mini"
                @blur="WideValueBlur(scope.row.wideValue,scope.row)"
                @keyup.enter.native="WideValueBlur(scope.row.wideValue,scope.row)"
              ></el-input>
            </div>-->
            <EditInput
        ...
        <el-table-column prop="lossValue" label="损耗" width="90">
          ...
          <template slot-scope="scope">
            <!-- <div
              @click="chengenum($event,scope.row,'lossValue')"
              :class="{validStyle:scope.row.lossValue.valid}"
            >
              <div
                class="tableTd"
                v-show="!scope.row.lossValue.isShow"
              >{{scope.row.lossValue.value}}</div>
              <el-input
                v-show="scope.row.lossValue.isShow"
                v-model="scope.row.lossValue.value"
                :id="scope.row.id+'___lossValue'"
                size="mini"
                @blur="NumBlur(scope.row.lossValue,scope.row)"
                @keyup.enter.native="NumBlur(scope.row.lossValue,scope.row)"
              ></el-input>
            </div>-->
            <EditInput
        ...
        <el-table-column prop="singleValue" label="单用量" width="120">
          ...
          <template slot-scope="scope">
            <div :class="{validStyle:scope.row.singleValue.valid}">
              ...
              <!-- <el-input
                v-show="scope.row.singleValue.isShow"
                v-model="scope.row.singleValue.value"
                :id="scope.row.id+'___singleValue'"
                size="mini"
              ></el-input>-->
            </div>
          </template>
        ...
        <el-table-column prop="formula" label="单用量计算公式" width="150">
          ...
          <template slot-scope="scope">
            <!-- <div
              @click="chengenum($event,scope.row,'formula')"
              :class="{validStyle:scope.row.formula.valid}"
            >
              <div class="tableTd" v-show="!scope.row.formula.isShow">{{scope.row.formula.value}}</div>
              <el-select
                v-show="scope.row.formula.isShow"
                v-model="scope.row.formula.key"
                :id="scope.row.id+'___formula'"
...
export default {
  ...
  async mounted() {
    ...
    var copyList = Storage.LStorage.get("copyList_No");
    if (!isEmpty(copyList)) {
  ...
  methods: {
    blurCheck(itemList, item, type, state) {
      ...
      switch (type) {
        case "materialCode": //物料代码
          if (state == 2) {
            //按下Enter请求数据
            this.getMaterielData(itemList, "materialCode");
          }
          break;
        case "materialName_": //物料代码
          if (state == 2) {
            //按下Enter请求数据
            this.getMaterielData(itemList, "materialName_");
          }
          break;
        case 2: //验证长、宽、件数、损耗
    ...
    setBomData(data) {
      if (data && data.childList) {
        this.TabBom = data.childList;
        this.formatBom(this.TabBom);
      } else {
        this.resetData();
    ...
    async addPutInStorage(state) {
      ...
      checkSelect.map(item => {
        ...
        if (item.editState) {
          if (
            isEmpty(item.materialCode.key) ||
            isEmpty(item.materialCode.value)
          ) {
            //验证物料代码
          ...
          if (!isEmpty(item.materialName.key)) {
            //部位ID
            param.partId = item.materialName.key;
          }
    ...
    getMaterielData(item, key) {
      //获取物料档案信息
      this.materielData = [];

      var queryData = [];
      if (item) {
        if (key && key == "materialCode") {
        ...
        if (key && key == "materialName_") {
          if (!isEmpty(item.materialName_.value)) {
            queryData.push({
              column: "materialName",
              content: item.materialName_.value,
              condition: 6
            });
          }
        }
    ...
    rowClick(row) {
      ...
      this.doItem.materialName_.valid = false;

      this.doItem.materialCode.value = row.materialCode;
      this.doItem.materialName_.value = row.materialName;
    ...
    doCopyOpt(list) {
      Storage.LStorage.set("copyList_No", list);
      this.copyList = list;
    }
  }
};
