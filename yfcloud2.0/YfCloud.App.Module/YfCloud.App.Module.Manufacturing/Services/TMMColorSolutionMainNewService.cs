...
namespace YfCloud.App.Module.Manufacturing.Services
{
    ...
    /// </summary>
    [UseDI(ServiceLifetime.Scoped, typeof(ITMMColorSolutionMainNewService))]
    public class TMMColorSolutionMainNewService : ITMMColorSolutionMainNewService
    {
        private readonly IDbContext _db;//数据库实例对象
        private readonly ILogger<TMMColorSolutionMainNewService> _log;//日志操作实例对象
        ...
        public TMMColorSolutionMainNewService(IDbContext dbContext, ILogger<TMMColorSolutionMainNewService> logger, IMapper mapper)
        {
            _db = dbContext;
            _log = logger;
            _mapper = mapper;
        ...
        public async Task<ResponseObject<List<TMMColorSolutionMainQueryNewModel>>> GetColorSolution(RequestGet requestObject, CurrentUser currentUser)
        {
            try
            {
                List<TMMColorSolutionMainQueryNewModel> queryData = null;//查询结果集对象
                ...
                //返回执行结果
                return ResponseUtil<List<TMMColorSolutionMainQueryNewModel>>.SuccessResult(queryData, totalNumber);
            }
            catch (Exception ex)
            {
                //返回查询异常结果
                return ResponseUtil<List<TMMColorSolutionMainQueryNewModel>>.FailResult(null, ex.Message);
            }
        }

        private DataTable GetReturnDataTable(List<TMMColorSolutionMainQueryNewModel> queryData, List<TMMColorSolutionDetailQueryNewModel> detailList)
        {
            ...
            foreach (var item in queryData)
            {
                ...
                newRow["ColorName"] = item.ColorName;
                foreach (var item2 in detailList.Where(p => p.MainId == item.ID))
                {
        ...
        /// <returns></returns>
        public async Task<ResponseObject<List<TMMPackageColorItem>>> GetTMMPackageColorItem(CurrentUser current)
        {
            try
            {
                List<TMMPackageColorItem> result = new List<TMMPackageColorItem>();

                var tBMPackageDbModel = _db.Instance.Queryable<TBMPackageDbModel>().Where(p => p.CompanyId == current.CompanyID && p.DeleteFlag == false).ToList();
                var ColorItem = await _db.Instance.Queryable<TMMPackageColorItemDbModel>().Where(p => p.CompanyId == current.CompanyID && p.DeleteFlag == false).ToListAsync();


                foreach (var item in tBMPackageDbModel)
                {
                    TMMPackageColorItem cItem = new TMMPackageColorItem();
                    cItem.Id = item.ID;
                    cItem.unixId = "M" + item.ID.ToString();
                    cItem.Children = new List<TMMPackageColorItem>();
                    cItem.ParentId = "";
                    cItem.Name = item.DicCode + item.DicValue;
                    foreach (var childItem in ColorItem.Where(p => p.PackageId == item.ID))
                    {
                        TMMPackageColorItem ccItem = new TMMPackageColorItem();
                        ccItem.Id = childItem.ID;
                        ccItem.unixId = "C" + childItem.ID.ToString();
                        ccItem.Name = childItem.ItemName;
                        cItem.Children.Add(ccItem);
                        cItem.ParentId = cItem.unixId;
                    }

                    result.Add(cItem);
                }

                return ResponseUtil<List<TMMPackageColorItem>>.SuccessResult(result);
            }
            catch (Exception ex)
            {
                return ResponseUtil<List<TMMPackageColorItem>>.FailResult(null, ex.Message);
            }
